...
export default function HealthDashboard({
  activeProfile = { id: 'user-arjun', name: 'Arjun Sharma' },
  reports = [],
  onSelectTab = () => {},
  onOpenUpload = () => {},
  onOpenEmergency = () => {}
      ...
      <div className="card-white p-6 space-y-4">
        <div className="flex items-center justify-between flex-wrap gap-2">
          ...
          <div className="flex items-center gap-2">
            ...
            >
              <span>View All Reports ({safeReports.length})</span>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3.5 pt-2">
          {recentReports.length === 0 ? (
            <div className="col-span-full py-8 text-center bg-slate-50 rounded-2xl border border-slate-200">
              <FileText className="w-8 h-8 text-slate-300 mx-auto mb-2" />
              <p className="text-xs text-slate-500 font-medium">No lab reports loaded yet.</p>
              <button
                onClick={onOpenUpload}
                className="btn-primary-green text-xs mt-3 py-1.5 px-3"
              >
                <UploadCloud className="w-3.5 h-3.5" /> Upload First Report
              </button>
            </div>
          ) : (
